...
export var Input = React.createClass ({
	...
	if (this.props.dims) {
	    gridVals = this.props.dims.split ('+') ;
	    delete iProps.dims ;
	}
...
export var Inputdrop = React.createClass ({
	...
	return (
		...
		<div className={"input-group col-md-" + gridVals [1]}
			 ...
			 >
		    <input className="form-control" {...iProps} />
		    <Dropdown_internal name={this.props.ddname}
			    defaultValue={this.props.ddDef}
...
export var Space = React.createClass ({
    render: function () {
	var gridVal = this.props.dims ?  this.props.dims : '1' ;

	return (
	    <div className={"col-md-" + gridVal} />
	) ;
    }
}) ;
 ...
 * AutoInput must have a `name` property and its value must correspond
 * to the name of a handler contained inside the Prompters (see above).
 *
 * @properties:
...
export var AutoInput = React.createClass ({
    /* state: current value of the input
...
export var InputXORdd = React.createClass ({
	...
	return (
		...
		<div className={"dropdown col-md-" + gridVals [2]}>
		    <Adropdown label="or"
...
export var DdEdit = React.createClass ({
	...
	if (this.props.edit === true) {
	    return (
		<Dropdown_internal superClass="dropdown"
			onChange={this.onChange}
			value={this.state.value}
			name={this.props.name}
			>
...
export var Editable_tr = React.createClass ({
    ...
    /* Called when the user remove the row */
    deleteRow: function () {
	var data = this.collectValues () ;
	...
	if (this.props.data.editable && this.props.data.editable === false) {
	    return ;
	}
...
export var Table = React.createClass ({
    ...
    componentWillMount: function () {
	this.retrieveValues (this.props.params) ;
    },

    componentWillReceiveProps: function (newProps) {
	this.retrieveValues (newProps.params) ;
    },

    renderHead: function () {
	function headerEl (mod, index) {
	    return (<th key={"th" + index}>{mod [0]}</th>) ;
	}
