...
%
% $Id: install.tex,v 1.11 2008-03-21 22:00:38 pda Exp $
%
% Historique
...
%    \end {quote}
    ...
    \begin {center}
	Version 1.4, ???? 2008 \\
	\verb|$Id: install.tex,v 1.11 2008-03-21 22:00:38 pda Exp $| \\
	\copyright
...
Web, à partir d'une base d'authentification externe à l'application
WebDNS, comme l'illustre la figure suivante~:
...
John Ousterhout. WebDNS a été développé avec la version 8.4.
...
une version supérieure ou égale à 7.4\footnote {La gestion des
adresses IPv6 dans WebDNS repose sur le type de données «~INET~»
de PostgreSQL, et plus spécifiquement sur son extension aux adresses
IPv6 à partir de PostgreSQL version 7.4.}.
...
\titreC {mod\_cas (authentification LDAP)}
...
\cbstart%1.4
\titreB {Installation de « htg » et du modèle « webdns »}
    \label {htg-webdns}
...
Si vous choisissez un répertoire d'installation différent de celui
où vous avez détarré les sources, prenez soin de copier les fichiers
«~.tcl~» ainsi que le fichier \texttt {Makefile}.

Vous pouvez également copier les pages de manuel (fichiers «~.n~»),
...
\titreC {Modifications du paquetage \texttt {arrgen}}

Aucune modification n'est nécessaire.

\titreC {Modifications du paquetage \texttt {pgsql}}

Aucune modification n'est nécessaire.

\titreC {Modifications du paquetage \texttt {auth}}
...
\begin {itemize}
    \item \texttt {trpw}, pour chiffrer un mot de passe
...
\verb|./pgauth/inst/httpd.conf|, notamment la première partie) dans
le fichier \texttt {httpd.conf} de configuration d'Apache, que vous
prendrez soin d'adapter~:

XXX : AUTH DOIT ETRE ACCESSIBLE A TOUS (CHGT MOT DE PASSE)
...
Le deuxième groupe que vous allez créer correspondra aux utilisateurs
de WebDNS que vous souhaitez installer (voir introduction).
...
La principale innovation de la version 1.4 de WebDNS est la remise
en cause du schéma d'authentification jusque là bâti sur WebAuth
et une base PostgreSQL.
...
\begin {itemize}
	...
	\begin {itemize}
	    ...
	    \item suppression d'une uerreur de syntaxe dans une
